...
export default function TemplateGeneratorPage() {
  ...
  const fetchHolderStats = useCallback(async () => {
    try {
      ...
      if (typeof count === 'number') {
        paymentLog.success(`✅ Found ${count} holders on-chain`);
        paymentLog.info(`👥 Found ${count} NFT holders`);
      } else {
  ...
  const fetchLatestDistribution = useCallback(
    async (context: string) => {
      ...
      paymentLog.info(`📡 Checking x402 distribution${label}...`);
      ...
      for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        try {
          ...
          if (latest && latest.txSignature && latest.txSignature !== lastDistributionSignature) {
            ...
            paymentLog.info('💵 Distribution details:');
            ...
            paymentLog.success(`🪙 Transfers confirmed on-chain: ${latest.txSignature}`);
            paymentLog.success(`✅ Distribution complete! Tx: ${latest.txSignature}`);
            paymentLog.info(`📊 Recorded distribution for ${X402_DEFAULT_NFT}`);
          ...
          if (attempt < maxAttempts) {
            paymentLog.info(`⏳ Distribution still finalizing (attempt ${attempt}/${maxAttempts})...`);
            await new Promise((resolve) => setTimeout(resolve, attemptDelay));
            continue;
          }

          paymentLog.info('⌛ Distribution history is still syncing. Check the x402 dashboard shortly for the latest entry.');
          return;
        } catch (error) {
          if (attempt < maxAttempts) {
            paymentLog.info(`🔁 Waiting on x402 history update (attempt ${attempt}/${maxAttempts})...`);
            await new Promise((resolve) => setTimeout(resolve, attemptDelay));
            continue;
          }
  ...
  const handlePaymentSuccess = (token: string) => {
    ...
    paymentLog.info(`🔍 Querying NFT holders for: ${X402_DEFAULT_NFT}`);
    paymentLog.success('✅ Payment verified. Triggering x402 distribution...');
    void (async () => {
      try {
  ...
  const handleCreditsPurchaseSuccess = (
    ...
    if (packPrice !== null) {
      paymentLog.info('💰 x402 payment webhook received');
      paymentLog.info(`💰 Processing x402 payment: ${packPrice.toFixed(6)} SOL`);
    } else {
      paymentLog.info('💰 x402 payment webhook received');
      paymentLog.info('💰 Processing x402 payment for credit purchase');
    }
    paymentLog.warning('⚠️ No signature validation (development mode)');
    paymentLog.info(`🎯 Target NFT: ${X402_DEFAULT_NFT}`);
    paymentLog.info('📋 NFT config: equal_split model');
    paymentLog.info(`🔍 Querying NFT holders for: ${X402_DEFAULT_NFT}`);
    paymentLog.success(`🪙 Credits purchased. New balance: ${newBalance} credits.`);
    void (async () => {
      try {
        await fetchHolderStats();
  ...
  const handleUseCredits = () => {
    ...
    setPaymentRequirement(null);
    setPaymentConsoleActive(true);
    paymentLog.success('✨ Using credits for this operation.');
    retryPendingOperation();
  };
  ...
  const handleOpenCreditsModal = () => {
    ...
    setPaymentConsoleActive(true);
    paymentLog.info('🛒 Opening credits purchase modal...');
    void captureLatestDistributionSignature();
  };
  ...
  const handleCompile = async () => {
    ...
    try {
      ...
      if (err instanceof PaymentRequiredError || err.name === 'PaymentRequiredError') {
        ...
        log.error('💳 Payment required for compilation', '💳');
        setIsActive(false);
        return;
      }
